...
# -------------------------------------------------------------
st.set_page_config(
    page_title="AI 반도체 심층 분석",
    page_icon="⚡",
    layout="wide"
)

st.title("⚡ AI 반도체 심층 분석")
st.markdown(
    "글로벌 AI 반도체 생태계(설계, 파운드리, 장비, HBM) 및 **고변동성 레버리지 ETF(2x/3x)**의 "
    "주가 추이, 상대 수익률, 펀더멘털 지표를 전문적으로 심층 분석합니다."
)
...
# -------------------------------------------------------------
# 데이터 수집 및 정제 함수 (Rate Limit & MultiIndex 안전 방어)
# -------------------------------------------------------------
...
if mode == "개별 종목/ETF 심층 분석":
    ...
    if df.empty or len(df) == 0:
        st.error("데이터를 불러올 수 없거나 호출 제한에 도달했습니다. 잠시 후 다시 시도해 주세요.")
    else:
        try:
